...
var higherOrLowerCalled = false;
var log = [];
var key;

var higherOrLower = function(guess) {
  if (higherOrLowerCalled) {
    return 'I can only be called once per script run';
  }

  higherOrLowerCalled = true;

  if (guess === key) return 'equal';
  if (guess > key) return 'lower';
  if (guess < key) return 'higher';
};
