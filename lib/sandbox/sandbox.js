...
var run = function(test, teamId, callback) {
  ...
  if (scriptExecutionIsThrottled(teamId)) {
    return callback('there must be 5s between each script run');
  }
...
var scriptExecutionIsThrottled = function(teamId) {
  if (!teamRunHistory[teamId]) {
    return false;
  }

  return Date.now() - teamRunHistory[teamId] < 5000;
};
