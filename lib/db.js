...
var completeChallenge3 = function(id) {
  ...
  if (teamIsSabotaged(team)) return { error: 'you are currently sabotaged – wait 10 minutes' };
...
var sabotageTeam = function(name) {
  ...
  save();
  return { msg: 'success – ' + team.name + ' are banned for the next 10 minutes' };
};
...
var teamIsSabotaged = function(team) {
  ...
  if (team.sabotaged) {
    if (new Date() - team.sabotagedAt > tenMinutes) {
